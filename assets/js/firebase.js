...
async function verificaFavorito() {
  var pokemon = document.getElementById("nomePokemonEscondido").value     //pegando o nome do pokemon sem formatação
  var usuario = firebase.auth().currentUser.uid                           //pegando o id do usuario 
  var achou = false                                                       //false apenas para iniciar 
  
  await firebase.database().ref("favoritos/" + usuario).once("value").then( (snapshot) => {   //consultando os favorito por usuario Ex: kamila

    snapshot.forEach(item => {                 //uso forEach pois e uma lista 
      var dados = item.val()                   //.val pegando os dados da minha collection para cada item
      
      //Pokemon EXISTE na collection favoritos
      if(dados.nome_pokemon == pokemon) {       //pego meu nome_pokemon dos meus dados e verifico se tenho se for verdadeiro entra no true 
         achou = true                          //flag(uso para me localizar)
      }      
    })
